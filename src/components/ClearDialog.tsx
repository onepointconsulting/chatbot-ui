...
export default function ClearDialog({}) {
  ...
  return <dialog data-model={true} id="clear-dialog">
    <div>Would you really like to delete the chat contents?</div>
    <input type="checkbox" checked={deleteHistory.value}
           onChange={() => deleteHistory.value = !deleteHistory.value}
           id={deleteHistoryCheckId}/> <label htmlFor={deleteHistoryCheckId} className="cursor-pointer">Delete history</label>
    <div className="flex justify-between mt-2">
      <button data-close-modal={true} onClick={onClose}>Close</button>
