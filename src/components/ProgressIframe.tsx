...
export default function ProgressIframe() {
  ...
  return (
    ...
    <div className="flex flex-row items-start h-full">
      <div className="">
        {showProgressChart.value && <img src={`${chartProgressUrl}/${session.id}?count=${data.length}`}
                                         className="w-full h-auto opacity-75"
                                         alt="Progress Indicator"/>}
        <button onClick={() => showProgressChart.value = !showProgressChart.value}>
          {showProgressChart.value &&
            <FaMinus className="absolute top-0 text-gray-500 right-0 m-2 cursor-pointer" size={20}
                     title="Hide progress"/>}
          {!showProgressChart.value &&
            <FaCcDiscover className="absolute text-white top-0 right-0 m-2 cursor-pointer mt-8" size={20}
                          title="Show progress"/>}
        </button>
      </div>
    </div>
