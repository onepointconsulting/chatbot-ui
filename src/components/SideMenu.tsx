...
export default function SideMenu({mobile = false, menuHeaderExpanded}: {
  mobile?: boolean,
  ...
  return (
    <div
      className={mobile ? 'fixed w-full left-0 z-40 bg-gray-50 top-14 h-full block sm:hidden' : 'mt-20 ml-2 md:ml-3 menu-icon'}>
      {menus.map((menu, index) => (
        <div className={mobile ? "ml-2 my-6" : "my-8"} key={index}>
          <Link to={menu.link} className="flex" onClick={(e) => {
            e.stopPropagation()
            if (mobile && menuHeaderExpanded) menuHeaderExpanded.value = false
          }}>
            {menu.icon}
